...
@willie.module.rate(60) #5 minute cooldown for one user
def gt(bot, trigger):
        ...
        if len(words) >= 2:
            if len(words) == 3:
                ...
                if any(b == cogname for b in cogs2):
                    ...
                    bot.msg(trigger.nick, 'Please use the correct name for the cog, please try again in one minute. a list can be found http://pastebin.com/3FxNti9T')
            else:
